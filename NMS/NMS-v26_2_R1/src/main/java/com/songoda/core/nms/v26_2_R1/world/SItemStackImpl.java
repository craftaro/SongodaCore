...
public class SItemStackImpl implements SItemStack {
    ...
    public void breakItem(Player player, int amount) {
        ...
        for (int i = 0; i < amount; ++i) {
            ...
            entityPlayer.level().addParticle(new ItemParticleOption(ParticleTypes.ITEM, CraftItemStack.asNMSCopy(this.item).getItem()), vec3d1.x, vec3d1.y, vec3d1.z, vec3d.x, vec3d.y + 0.05D, vec3d.z);
        }
    }
}
