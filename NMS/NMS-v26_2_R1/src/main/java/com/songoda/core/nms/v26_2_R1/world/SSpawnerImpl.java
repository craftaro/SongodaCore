...
public class SSpawnerImpl implements SSpawner {
    ...
    public LivingEntity spawnEntity(EntityType type, String particleType, SpawnedEntity spawned, Set<XMaterial> canSpawnOn) {
        ...
        for (int i = 0; i < 50; ++i) {
            ...
            // EntityType.create(ValueInput, Level, EntitySpawnReason) no longer exists in 26.2;
            // the reason now has to be wrapped in an EntitySpawnRequest.
            Optional<Entity> optionalEntity = net.minecraft.world.entity.EntityType.create(
                    CompoundTagConverter.toInput(compound, world),
                    world,
                    new EntitySpawnRequest(EntitySpawnReason.SPAWNER, false)
            );
            ...
            entityInsentient.finalizeSpawn(world, damageScaler, EntitySpawnReason.SPAWNER, null);
