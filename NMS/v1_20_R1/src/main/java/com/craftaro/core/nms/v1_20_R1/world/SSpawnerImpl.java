...
public class SSpawnerImpl implements SSpawner {
    ...
    private boolean canSpawn(ServerLevel world, Mob entityInsentient, Location location, Set<XMaterial> canSpawnOn) {
        ...
        XMaterial spawnedIn = XMaterial.matchXMaterial(location.getBlock().getType());
        XMaterial spawnedOn = XMaterial.matchXMaterial(location.getBlock().getRelative(BlockFace.DOWN).getType());

        if (spawnedIn != XMaterial.AIR &&
                spawnedIn != XMaterial.WATER &&
        ...
        for (XMaterial material : canSpawnOn) {
            ...
            if (spawnedOn == material || material == XMaterial.AIR) {
                return true;
            }
        }
